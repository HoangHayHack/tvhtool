...
\033[1;32m[\033[1;31m*\033[1;32m]\033[1;33m ➩ \033[1;32mBạn Đang Sử Dụng \033[1;31m: \033[1;36mTool TDS Facebook
...
class Facebook_Api(object):
        ...
        try:
            ...
            if proxy:
                try:
                    ...
                    print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Khởi Tạo Proxy : {str(e)}"))
                    self.proxies = None
                    
            url = requests.get(f'https://web.facebook.com/{self.actor_id}', headers=self.headers, proxies=self.proxies).url
    ...
    def info(self):
        get = requests.get('https://web.facebook.com/me',headers=self.headers, proxies=self.proxies).text
        try:
    ...
    def Checkspam(self):
        ...
        response = requests.post('https://web.facebook.com/api/graphql/',headers=self.headers, data=data, proxies=self.proxies)
    ...
    def recaptchav2(self, sitekey, siteurl):
        ...
        try:
            ...
            if response.get("errorId", 0) != 0:
                print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Tạo Task : {response}"))
                return False, None
            return True, response.get("taskId")
        except requests.RequestException as e:
            print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Gửi Yêu Cầu Tạo Task : {e}"))
            return False, None
    ...
    def get_result(self, task_id, max_retries=10, delay=0):
        ...
        for x in range(max_retries):
            try:
                ...
                if response.get("errorId", 0) != 0:
                    print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Lấy Kết Quả : {response}"))
                    return False, None
                if response.get("status") == "ready":
                    return True, response["solution"]["gRecaptchaResponse"]
                sleep(delay)
            except requests.RequestException as e:
                print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Lấy Kết Quả Captcha : {e}"))
                return False, None
        
        print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Hết Số Lần Thử, Không Có Kết Quả !"))
        return False, None
...
class TraoDoiSub_Api(object):
    ...
    def info(self):
        ...
        if 'success' in response.text:
            self.cookie = response.headers.get('Set-Cookie') or response.headers.get('set-cookie')
            headers = {
    ...
    def get_g_recaptcha_response(self, apikey):
        try:
            ...
            if response.status_code != 200:
                print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Lấy SiteKey !"))
                return False, None
            ...
            print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Lỗi Khi Giải reCAPCHA"))
            return False
...
def Nhap_Cookie():
    ...
    while True:
        demck += 1
        ck = input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập Cookie Facebook Thứ {demck} : "))
        if ck == '' and demck > 1:
            break
        fb = Facebook_Api(ck)
        info = fb.info()
        if 'success' in info:
            ...
            uid = info['id']
            print("\033[1;97m════════════════════════════════════════════════")
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Id Facebook : {uid} | Tên Tài Khoản : {name}"))
            listck.append(ck)
            print("\033[1;97m════════════════════════════════════════════════")
        else:
            print("\033[1;97m════════════════════════════════════════════════")
            print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Trạng Thái Acc : [Die]"))
            print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Tin Nhắn : Đăng Nhập Thất Bại"))
            demck-=1
            print("\033[1;97m════════════════════════════════════════════════")
    return listck

def Nhap_Setting():
    apikey = input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập Api Key 3xcaptcha Để Auto Add Cấu Hình (Enter Để Bỏ Qua Nếu Không Cần) : "))
    min = int(input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập Delay Min : ")))
    max = int(input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập Delay Max : ")))
    nvblock = int(input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Sau Bao Nhiêu Nhiệm Vụ Thì Chống Block : ")))
    delaybl = int(input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Sau Bao Nhiêu {nvblock} Nhiệm Vụ Thì Nghỉ Ngơi (Giây) : ")))
    doinick = int(input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Sau Bao Nhiêu Nhiệm Vụ Thì Đổi Nick : ")))
    nhiemvuloi = int(input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Lỗi Bao Nhiêu Nhiệm Vụ Thì Xóa Cookie : ")))
    ...
    with open('Setting_TDS_Fb.json', 'w') as f:
        json.dump(config, f)
    return config
...
def Main():
    ...
    print(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Proxy Dạng : username:password@host:port"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Ví Dụ : tvhtool:qwerty@127.0.0.1:1080"))
    print("\033[1;97m════════════════════════════════════════════════")

    filename = input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập File Txt Chứa Proxy (Enter Để Bỏ Qua Nếu Không Cần) : "))
    print("\033[1;97m════════════════════════════════════════════════")
    sleep(1)

    if not filename.strip():
        print(Colorate.Diagonal(Colors.green_to_white, "[♤] ➩ Bạn Không Nhập Proxy, Tiếp Tục !"))
        sleep(1.5)
        proxy_list = []
        current_proxy = None
        myip = None
        ...
        if proxy_list:
            ...
            if not is_live:
                print(Colorate.Diagonal(Colors.red_to_white, "[♤] ➩ Proxy Không Hoạt Động !"))
                myip = None
            else:
                print(Colorate.Diagonal(Colors.green_to_white, "[♤] ➩ Proxy Hoạt Động !"))
                myip = check['data'][0]['ip']

    os.system("cls" if os.name == "nt" else "clear")
    banner()
    while True:
        if os.path.exists('Acc_TDS.txt'):
            with open('Acc_TDS.txt', 'r') as f:
                username, password = f.read().split(' | ')
            tds = TraoDoiSub_Api(username, password, current_proxy)
            profile = tds.info()
            try:
                print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (1) : Để Chạy Tài Khoản {profile[1]}"))
                print(Colorate.Diagonal(Colors.green_to_white, "[♤] ➩ Nhập Số (2) : Nhập Tài Khoản Trao Đổi Sub Mới"))
                print("\033[1;97m════════════════════════════════════════════════")
                chon = input(Colorate.Diagonal(Colors.blue_to_white, "[♤] ➩ Nhập Số : "))
                print("\033[1;97m════════════════════════════════════════════════")

                if chon == '2':
                    os.remove('Acc_TDS.txt')
                elif chon == '1':
                    pass
                else:
                    print(Colorate.Diagonal(Colors.red_to_white, "[♤] ➩ Vui Lòng Chọn Đúng !"))
                    print("\033[1;97m════════════════════════════════════════════════")
                    continue
            except:
                print(Colorate.Diagonal(Colors.red_to_white, "[♤] ➩ Trạng Thái Acc : [Die]"))
                print(Colorate.Diagonal(Colors.red_to_white, "[♤] ➩ Tin Nhắn : Đăng Nhập Thất Bại !"))
                print("\033[1;97m════════════════════════════════════════════════")
                os.remove('Acc_TDS.txt')
        if not os.path.exists('Acc_TDS.txt'):
            username, password = input(Colorate.Diagonal(Colors.blue_to_white, "[♤] ➩ Nhập Tài Khoản TDS : ")), input(Colorate.Diagonal(Colors.blue_to_white, "[♤] ➩ Nhập Mật Khẩu TDS : "))
            print("\033[1;97m════════════════════════════════════════════════")
            with open('Acc_TDS.txt', 'w') as f:
        ...
        try:
            user = profile[1]
            xu = profile[2]
            print(Colorate.Diagonal(Colors.green_to_white, "[♤] ➩ Trạng Thái Acc : [Live]"))
            print(Colorate.Diagonal(Colors.green_to_white, "[♤] ➩ Tin Nhắn : Đăng Nhập Thành Công !"))
            break
        except:
            print(Colorate.Diagonal(Colors.red_to_white, "[♤] ➩ Trạng Thái Acc : [Die]"))
            print(Colorate.Diagonal(Colors.red_to_white, "[♤] ➩ Tin Nhắn : Đăng Nhập Thất Bại !"))
    ...
    print("\033[1;97m════════════════════════════════════════════════")
    while True:
        if os.path.exists('Cookie_Facebook.txt'):
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (1) : Sử Dụng Cookie Facebook Đã Lưu"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (2) : Nhập Cookie Facebook Mới"))
            print("\033[1;97m════════════════════════════════════════════════")
            chon = input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập Số : "))
            print("\033[1;97m════════════════════════════════════════════════")
            if chon == '1':
                sleep(1.5)
                print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Lấy Cookie Facebook Đã Được Lưu Thành Công !"))
                print("\033[1;97m════════════════════════════════════════════════")
                sleep(1.5)
                with open('Cookie_Facebook.txt', 'r') as f:
                    listck = json.loads(f.read())
                    break
                    
            elif chon == '2':
                os.remove('Cookie_Facebook.txt')
            else:
                print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Vui Lòng Chọn Đúng !"))
                print("\033[1;97m════════════════════════════════════════════════")
                continue
        if not os.path.exists('Cookie_Facebook.txt'):
            listck = Nhap_Cookie()
            with open('Cookie_Facebook.txt', 'w') as f:
                json.dump(listck, f)
            break
    os.system("cls" if os.name == "nt" else "clear")
    banner()
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Tên Tài Khoản TDS : {user}"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Xu TDS Hiện Tại : {str(format(int(xu),","))}"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Số Tài Khoản Facebook : {len(listck)}"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Trạng Thái Của Proxy : {myip if myip else 'Không Có'}"))
    print("\033[1;97m════════════════════════════════════════════════")
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (1) : Để Chạy Nhiệm Vụ Cảm Xúc Vip"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (2) : Để Chạy Nhiệm Vụ Cảm Xúc Comment Vip"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (3) : Để Chạy Nhiệm Vụ Share Vip"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (4) : Để Chạy Nhiệm Vụ Follow Vip"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (5) : Để Chạy Nhiệm Vụ Like Page Vip"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (6) : Để Chạy Nhiệm Vụ Tham Gia Group"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (7) : Để Chạy Nhiệm Vụ Share Thường"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (8) : Để Chạy Nhiệm Vụ Like Thường"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Nhập Số (9) : Để Chạy Nhiệm Vụ Cảm Xúc Thường"))
    print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Có Thể Chọn Nhiều Nhiệm Vụ (Ví Dụ : 12346...)"))
    print("\033[1;97m════════════════════════════════════════════════")
    nhap = input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Nhập Số : "))
    if '' in nhap:
        nhap = nhap.replace('','')
    listnv.append(nhap)
    print("\033[1;97m════════════════════════════════════════════════")
    if os.path.exists('Setting_TDS_Fb.json'):
        with open('Setting_TDS_Fb.json', 'r') as f:
            config = json.load(f)
            ...
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Api Key 3xCapcha : {apikey if apikey else 'Không Có'}"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Delay Min : {min} Giây"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Delay Max : {max} Giây"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Sau {nvblock} Nhiệm Vụ Thì Chống Block"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Sau {nvblock} Nhiệm Vụ Thì Nghỉ Ngơi {delaybl} Giây"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Sau {doinick} Nhiệm Vụ Thì Đổi Nick"))
            print(Colorate.Diagonal(Colors.green_to_white, f"[♤] ➩ Lỗi {nhiemvuloi} Nhiệm Vụ Thì Đổi Nhiệm Vụ"))
            print("\033[1;97m════════════════════════════════════════════════")
        ...
        doinick = config['doinick']
        nhiemvuloi = config['nhiemvuloi']
    chonan = input(Colorate.Diagonal(Colors.blue_to_white, f"[♤] ➩ Bạn Có Muốn Ẩn Id Facebook Không ? (y/n) : "))
    print("\033[1;97m════════════════════════════════════════════════")
    sleep(1.5)
    
    while True:
        if len(listck) == 0:
            print(Colorate.Diagonal(Colors.red_to_white, f"[♤] ➩ Đã Xoá Tất Cả Cookie, Vui Lòng Nhập Lại !"))
            print("\033[1;97m════════════════════════════════════════════════")
            listck = Nhap_Cookie()
            with open('Cookie_Facebook.txt', 'w') as f:
                json.dump(listck, f)
        for ck in listck:
            ...
            if proxy_list:
                ...
                if is_live == False:
                    ...
                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mProxy Hoạt Động !                            ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    myip = check['data'][0]['ip']
            ...
            if cauhinh == True:
                os.system("cls" if os.name == "nt" else "clear")
                banner()
                print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mId Facebook \033[1;31m: \033[1;36m{uid2} \033[1;97m| \033[1;32mTên Tài Khoản \033[1;31m: \033[1;36m{name}')
                print("\033[1;97m════════════════════════════════════════════════")
            else:
                if apikey:
                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Thêm Id Facebook \033[1;31m: \033[1;36m{uid} \033[1;32m| Tên Tài Khoản \033[1;31m: \033[1;36m{name}')
                    ...
                    if get_g_recaptcha_response[0] == True:
                        ...
                        if add_uid[0] == True:
                            cauhinh = tds.facebook_configuration(uid)
                            if cauhinh == True:
                                print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mId Facebook \033[1;31m: \033[1;36m{uid2} \033[1;32m| Tên Tài Khoản \033[1;31m: \033[1;36m{name}')
                            else:
                                print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mCấu Hình Thất Bại Id Facebook \033[1;31m: \033[1;36m{uid} \033[1;32m| Tên Tài Khoản \033[1;31m: \033[1;36m{name}')
                                listck.remove(ck)
                                continue
                        else:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mThêm Cấu Hình Thất Bại Id Facebook \033[1;31m: \033[1;36m{uid} \033[1;32m| Tên Tài Khoản \033[1;31m: \033[1;36m{name}')
                            listck.remove(ck)
                            continue
                    else:
                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mThêm Cấu Hình Thất Bại Id Facebook \033[1;31m: \033[1;36m{uid} \033[1;32m| Tên Tài Khoản \033[1;31m: \033[1;36m{name}')
            ...
            while True:
                ...
                if '1' in nhiemvu:
                    ...
                    if listcx == False:
                        ...
                        if listcx['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mĐang Get Nhiệm Vụ Cảm Xúc, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                        ...
                        if len(list_nv) == 0:
                            ...
                            for x in list_nv:
                                ...
                                if like == False:
                                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mFailed {type} : {id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                    Delay(1)
                                    loireaction += 1
                                else:
                                    nhan = tds.get_xu_vip('facebook_reaction', code)
                                    if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                        xu = nhan['data']['xu']
                                        msg = nhan['data']['msg']
                                        loireaction = 0
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;37m{type} \033[1;32m| \033[1;33m{id2} \033[1;32m| \033[1;34m{msg}')
                                        if dem % doinick == 0:
                                            tvhtool = 1
                                            break
                                        if dem % nvblock == 0:
                                ...
                                if loireaction >= nhiemvuloi:
                                    ...
                                    checktt = fb2.info()
                                    if 'error' in checktt:
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mCookie Tài Khoản {name} Đã Bị Out Hoặc Checkpoint !                ')
                                        listck.remove(ck)
                                        tvhtool = 1
                                        break
                                    else:
                ...
                if '2' in nhiemvu:
                    ...
                    if listcxcmt == False:
                        ...
                        if listcxcmt['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Get Nhiệm Vụ Follow, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                        ...
                        if len(list_nv) == 0:
                            ...
                            for x in list_nv:
                                ...
                                if like == False:
                                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mLỗi {type} Comment \033[1;31m: \033[1;36m{id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                    Delay(1)
                                    loicxcmt += 1
                                else:
                                    ...
                                    if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                        ...
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;37m{type} COMMENT \033[1;32m| \033[1;33m{id2} \033[1;32m| \033[1;34m{msg}')
                                        if dem % doinick == 0:
                                            tvhtool = 1
                                            break
                                        if dem % nvblock == 0:
                ...
                if '3' in nhiemvu:
                    ...
                    if listshare == False:
                        ...
                        if listshare['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            ...
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Get Nhiệm Vụ Share, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                        ...
                        for x in list_nv:
                            ...
                            like = fb.share(id)
                            if like == False:
                                print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mFailed Share : {id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                Delay(1)
                                loishare += 1
                            else:
                                ...
                                if isinstance(nhan, (dict, str)) and 'success' in nhan:                                	
                                    ...
                                    dem+=1
                                    time = datetime.now().strftime("%H:%M:%S")
                                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;97mSHARE \033[1;32m| \033[1;33m{id2} \033[1;32m| \033[1;34m{msg}')
                                    if dem % doinick == 0:
                                        tvhtool = 1
                                        break
                                    if dem % nvblock == 0:
                                ...
                                if loishare >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mTài Khoản {name} Đã Bị Block Share !                           ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    ...
                    if listfl == False:
                        ...
                        if len(list_nv) == 0:
                            ...
                            for x in list_nv:
                                ...
                                like = fb.follow(id)
                                if like == False:
                                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mFailed Follow : {id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                    Delay(1)
                                    loifollow += 1
                                else:
                                    ...
                                    loifollow = 0
                                    time = datetime.now().strftime("%H:%M:%S")
                                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;97mFOLLOW \033[1;32m| \033[1;34m{id2} \033[1;32m| \033[1;35m{cache}/5')
                                    if dem % doinick == 0:
                                        tvhtool = 1
                                        break
                                    if dem % nvblock == 0:
                                    ...
                                    if cache >= 5:
                                        ...
                                        if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                            ...
                                            time = datetime.now().strftime("%H:%M:%S")
                                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31m{time} \033[1;32m| \033[1;33mFOLLOW \033[1;32m| \033[1;34mNhận Xu \033[1;32m| \033[1;35m{job_success}/{cache} \033[1;32m| \033[1;37m{msg} \033[1;32m| \033[1;30m{str(format(int(xu),","))}')
                                ...
                                if loifollow >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mTài Khoản {name} Đã Bị Block Follow !                            ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    ...
                    if listpage == False:
                        ...
                        if len(list_nv) == 0:
                            ...
                            for x in list_nv:
                                ...
                                if like == False:
                                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36mFailed LikePage : {id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                    Delay(1)
                                    loipage+=1
                                else:
                                    ...
                                    loipage = 0
                                    time = datetime.now().strftime("%H:%M:%S")
                                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;97mLIKEPAGE \033[1;32m| \033[1;33m{id2} \033[1;32m| \033[1;35m{cache}/5')
                                    if dem % doinick == 0:
                                        tvhtool = 1
                                        break
                                    if dem % nvblock == 0:
                                    ...
                                    if cache >= 5:
                                        ...
                                        if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                            ...
                                            job_success = nhan['data']['job_success']
                                            time = datetime.now().strftime("%H:%M:%S")
                                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31m{time} \033[1;32m| \033[1;33mLIKEPAGE \033[1;32m| \033[1;34mNhận Xu \033[1;32m| \033[1;35m{job_success}/{cache} \033[1;32m| \033[1;37m{msg} \033[1;32m| \033[1;30m{str(format(int(xu),","))}')
                                ...
                                if loipage >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mTài Khoản {name} Đã Bị Block Like Page !                            ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                ...
                if '6' in nhiemvu:
                    ...
                    if listgr == False:
                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mKhông Get Được Nhiệm Vụ Group !                           ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    elif 'error' in listgr:
                        if listgr['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listgr['countdown']
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Get Nhiệm Vụ Group, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                        ...
                        list_nv = listgr
                        if len(list_nv) == 0:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mHết Nhiệm Vụ Group !                           ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                        else:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mTìm Thấy {len(list_nv)} Nhiệm Vụ Group                     ', end = '\r')
                        for x in list_nv:
                            idpost = x['id']
                            id2 = idpost[:3]+'#'*(len(idpost)-6)+idpost[-3:]
                            like = fb.group(idpost)
                            if like == False:
                                print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36mFailed Group : {idpost}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                Delay(1)
                                loigr += 1
                            else:
                                ...
                                if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                    ...
                                    dem+=1
                                    time = datetime.now().strftime("%H:%M:%S")
                                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;33mGroup \033[1;32m| \033[1;34m{id2} \033[1;32m| \033[1;35m{cache}/5')
                                    if dem % doinick == 0:
                                        tvhtool = 1
                                        break
                                    if dem % nvblock == 0:
                                ...
                                if loigr >= nhiemvuloi:
                                    ...
                                    checktt = fb2.info()
                                    if 'error' in checktt:
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31m{time} \033[1;32m| \033[1;33mGroup \033[1;32m| \033[1;34mNhận Xu \033[1;32m| \033[1;35m{job_success}/{cache} \033[1;32m| \033[1;37m{msg} \033[1;32m| \033[1;30m{str(format(int(xu),","))}')
                                        listck.remove(ck)
                                        tvhtool = 1
                                        break
                                    else:
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mTài Khoản {name} Đã Bị Block Group !                            ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                ...
                if '7' in nhiemvu:
                    ...
                    if listshare == False:
                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mKhông Get Được Nhiệm Vụ Share Thường !                           ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    elif 'error' in listshare:
                        if listshare['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listshare['countdown']
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Get Nhiệm Vụ Share Thường, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                            print(listshare['error'], end = '\r'); sleep(0); print('                                                       ', end = '\r')
                    else:
                        list_nv = listshare
                        if len(list_nv) == 0:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mHết Nhiệm Vụ Share Thường !                           ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                        else:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mTìm Thấy {len(list_nv)} Nhiệm Vụ Share Thường                     ', end = '\r')
                        for x in list_nv:
                            idpost = x['id']
                            id2 = idpost[:3]+'#'*(len(idpost)-6)+idpost[-3:]
                            share = fb.share(idpost)
                            if share == False:
                                print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mLỗi Share \033[1;31m: \033[1;36m{idpost}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                Delay(1)
                                loisharet += 1
                            else:
                                ...
                                if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                    ...
                                    dem+=1
                                    time = datetime.now().strftime("%H:%M:%S")
                                    print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;97mShare \033[1;32m| \033[1;33m{id2} \033[1;32m| \033[1;35m{cache}/5')                                    
                                    if dem % doinick == 0:
                                        tvhtool = 1
                                        break
                                    if dem % nvblock == 0:
                ...
                if '8' in nhiemvu:
                    ...
                    if listlike == False:
                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mKhông Get Được Nhiệm Vụ Like Thường                          ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    elif 'error' in listlike:
                        if listlike['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listlike['countdown']
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Get Nhiệm Vụ Like Thường, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                        ...
                        list_nv = listlike
                        if len(list_nv) == 0:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mHết Nhiệm Vụ Like Thường                          ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                        else:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mTìm Thấy {len(list_nv)} Nhiệm Vụ Like Thường                    ', end = '\r')
                            for x in list_nv:
                                idpost = x['id']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                id2 = id[:3]+'*'*(len(id)-6)+id[-3:]
                                ...
                                if like == False:
                                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mFailed Like Thường : {id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                    Delay(1)
                                    loiliket+=1
                                else:
                                    ...
                                    if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                        ...
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;33mLike Thường \033[1;32m| \033[1;34m{id2} \033[1;32m| \033[1;35m{cache}/5')
                                        if dem % doinick == 0:
                                            tvhtool = 1
                                            break
                                        if dem % nvblock == 0:
                                ...
                                if loiliket >= nhiemvuloi:
                                    ...
                                    if 'error' in checktt:
                                        ...
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mTài Khoản {name} Đã Bị Block Like Thường !                           ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                ...
                if '9' in nhiemvu:
                    ...
                    if listlike == False:
                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mKhông Get Được Nhiệm Vụ Cảm Xúc Thường                          ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                    elif 'error' in listlike:
                        if listlike['error'] == 'Thao tác quá nhanh vui lòng chậm lại':
                            count = listlike['countdown']
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mĐang Get Nhiệm Vụ Cảm Xúc Thường, Thời Gian Chờ \033[1;31m: \033[1;36m{str(round(count, 3))}              ', end = '\r'); sleep(0); print('                                                       ', end = '\r')
                            Delay(count)
                        else:
                            print(listlike['error'], end = '\r'); sleep(0); print('                                                       ', end = '\r')
                    else:
                        list_nv = listlike
                        if len(list_nv) == 0:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mHết Nhiệm Vụ Cảm Xúc Thường                          ', end = '\r');sleep(0); print('                                                        ', end = '\r')
                        else:
                            print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;32mTìm Thấy {len(list_nv)} Nhiệm Vụ Cảm Xúc Thường                    ', end = '\r')
                            for x in list_nv:
                                idpost = x['id']
                                type = x['type']
                                id = idpost.split('_')[1] if '_' in idpost else idpost
                                ...
                                like = fb.reaction(id, type)
                                if like == False:
                                    print(f"\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mLỗi {type} \033[1;32m: \033[1;36m{id}            ", end = '\r'); sleep(0); print('                                                       ', end = '\r')
                                    Delay(1)
                                    loiliket+=1
                                else:
                                    ...
                                    if isinstance(nhan, (dict, str)) and 'success' in nhan:
                                        ...
                                        dem+=1
                                        time = datetime.now().strftime("%H:%M:%S")
                                        print(f'\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;36m({dem}) \033[1;32m| \033[1;31m{time} \033[1;32m| \033[1;33mThường \033[1;32m| \033[1;34m{id2} \033[1;32m| \033[1;35m{cache}/5')
                                        if dem % doinick == 0:
                                            tvhtool = 1
                                            break
                                        if dem % nvblock == 0:
                                ...
                                if isinstance(loiliket, bool) or isinstance(nhiemvuloi, bool):
                                    print("\033[1;32m[\033[1;31m♤\033[1;32m]\033[1;33m ➩ \033[1;31mKhông Thể So Sánh Nhiệm Vụ Vì Giá Trị Lỗi, Bỏ Qua !")
                                    continue
